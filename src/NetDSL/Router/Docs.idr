...
private
enabledDisabled : Bool -> String
enabledDisabled True = "enabled"
enabledDisabled False = "disabled"
...
attachmentJSON Unattached = obj [("kind",jsonString "unattached")]
...
interfaceJSON (i,f) = obj (entity i f.name f.source ++
  [("attachment",attachmentJSON f.attachment.value),
   ...
   ("gateway",maybe "null" (jsonString . showIPv4 . value) f.gateway),
   ("dnsServers",jsonArray (map (jsonString . showIPv4 . value) f.dnsServers)),
...
poolJSON d = case d.pool of
  Nothing => "null"
  Just (a,b) => obj [("first",jsonString (showIPv4 a.value)),("last",jsonString (showIPv4 b.value)),("count",show (b.value.number-a.value.number+1)),("activeLeaseCount",show (leaseCount d))]
...
routerJSON c = obj
  [("physicals",jsonArray (map (\(i,p) => obj (entity i p.name p.source)) (indexed c.physicals))),
   ("bridges",jsonArray (map (\(i,b) => obj (entity i b.name b.source ++ [("members",jsonArray (map (attachmentJSON . value) b.members)),("stp",maybe "null" jsonBool b.stp)])) (indexed c.bridges))),
   ...
   ("wifiInterfaces",jsonArray (map (\(i,a) => obj (entity i a.name a.source ++ [("radioRef",show a.radio.value.index),("interfaceRef",show a.ifaceRef.value.index),("credentialRef",maybe "null" (jsonString . secretURI) a.credential),("settings",jsonWiFiOptions a.settings)])) (indexed c.wifiInterfaces))),
...
routerGraph nodePrefix c =
  ...
  concatMap (\(i,f) => node (ifaceId i) (f.name ++ " / " ++ maybe "?" showInterfaceProtocol f.settings.protocol) ++ (case f.attachment.value of Unattached => ""; _ => edge (linkId (attachmentId c f.attachment.value)) (ifaceId i))) (indexed c.interfaces) ++
  ...
  concatMap (\(i,a) => node (nodePrefix ++ "ap" ++ show i) (a.name ++ " / " ++ fromMaybe "" a.settings.ssid ++ if a.settings.disabled == Just True then " (disabled)" else "") ++
    edge (nodePrefix ++ "radio" ++ show a.radio.value.index) (nodePrefix ++ "ap" ++ show i) ++ edge (nodePrefix ++ "ap" ++ show i) (ifaceId a.ifaceRef.value.index)) (indexed c.wifiInterfaces)
  where
    linkId : Nat -> String
...
routerMarkdown routerName c =
  ...
  table ["Link","Kind","Members","STP"]
    (map (\b => [b.name,"bridge",join ", " (map (attachmentName c . value) b.members),maybe "default" enabledDisabled b.stp]) c.bridges ++
     map (\b => [b.name,"bond",join ", " (map (\m => maybe "invalid" name (lookupAt m.value.index c.physicals)) b.members),"n/a"]) c.bonds) ++
  table ["Interface","Attachment","Protocol","IPv4","IPv6 assignment","Gateway","DNS"]
    (map (\i => [i.name,attachmentName c i.attachment.value,maybe "?" showInterfaceProtocol i.settings.protocol,
      join ", " (map (\a => showIPv4 a.value.address ++ "/" ++ show a.value.subnet.width) i.addresses),maybe "none" (\n => "/" ++ show n) i.settings.ipv6Assignment,maybe "none" (showIPv4 . value) i.gateway,join ", " (map (showIPv4 . value) i.dnsServers)]) c.interfaces) ++
  table ["DHCP interface","Pool","Active leases","IPv6 server","Router advertisements"]
    (map (\d => [maybe "invalid" name (lookupAt d.ifaceRef.value.index c.interfaces),case d.pool of Nothing => "none"; Just (a,b) => showIPv4 a.value ++ " .. " ++ showIPv4 b.value,show (leaseCount d),
  ...
  table ["Wi-Fi interface","Mode","SSID","Radio","Interface","Security","Enabled","WDS","Hidden","BSSID","Credential reference"]
    (map (\a => [a.name,maybe "?" showWiFiMode a.settings.mode,fromMaybe "" a.settings.ssid,maybe "invalid" name (lookupAt a.radio.value.index c.radios),maybe "invalid" name (lookupAt a.ifaceRef.value.index c.interfaces),maybe "?" showSecurity a.settings.security,if a.settings.disabled == Just True then "no" else "yes",maybe "default" yesNo a.settings.wds,maybe "default" yesNo a.settings.hidden,fromMaybe "unspecified" a.settings.bssid,maybe "none" secretURI a.credential]) c.wifiInterfaces) ++
